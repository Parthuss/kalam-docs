...
## [T+0:00] Light theme, not dark
Default create-next-app boilerplate ships a dark-mode media query and an unlayered `body { background: var(--background) }` rule in globals.css. That unlayered rule silently overrides every Tailwind utility class on `<body>` (bg-white, text-slate-900, etc.) regardless of specificity — Tailwind v4 emits utilities inside `@layer`, and per the CSS cascade layers spec, any unlayered style beats a layered one no matter how specific the layered selector is. Removed the boilerplate background/foreground variables and dark-mode block entirely; committing to a single light theme, `color-scheme: light` set explicitly.

## [T+0:00] Bug caught in manual testing: viewer saw "Save failed" on a read-only doc
Tiptap's `onUpdate` fires on transactions beyond real content edits (e.g. selection-only), so a Viewer clicking into a read-only document triggered a PATCH attempt that the server correctly 403'd — but the UI still rendered "Save failed — retry" to someone who never tried to save anything. Confirmed the underlying edit was correctly blocked (content was unchanged), but the error surface was wrong. Fixed by gating `onUpdate` on both `editable` and `transaction.docChanged`, and only rendering the save indicator at all when `editable`. Found by logging in as three different seeded roles and clicking through each one on the live app, not by code review — worth calling out in the AI-workflow note as a case where manual multi-account verification caught something a passing typecheck would not.
...
## [T+0:00] Post-build user testing found real gaps a passing test suite didn't
Hands-on testing by the actual candidate (not me) on the live deployment surfaced three points: (1) the editor had no visible "page" container — text floated directly on white, unclear where to type until clicking; (2) toggling Bold/Underline/Italic off via the toolbar seemed to do nothing to already-typed text; (3) the default indigo/purple accent color read as generic AI-generated styling.

Investigated before touching any code: read a live document's exact ProseMirror DOM via a read-only JS query (`document.querySelector('.ProseMirror').innerHTML`), no mutation. The marks were nested exactly as toolbar clicks would produce (underline → +bold → +italic, correctly stacking on new characters). Confirmed via web search against Tiptap's own docs: toggling a mark with the cursor collapsed (nothing selected) only changes the *stored mark for the next character typed* — it does not retroactively affect already-typed text. This is documented Tiptap default behavior and matches Google Docs exactly (also confirmed: pressing Enter after a Heading in Docs resets to Normal text too, same as this app). **Not a bug.** Constructive fix instead of a false "bug fix": added a **Clear Formatting** button (`unsetAllMarks().clearNodes()` on the selection) so stripping stacked marks takes one click — every mainstream editor has one and this app didn't.

Items (1) and (3) were real, confirmed by direct file read: `editor.tsx`'s content area had zero card/border/shadow, and `indigo-600` was reused identically across 8 files for primary buttons, badges, focus rings, and link hovers with no hierarchy — exactly what reads as generic.

Used a Plan agent to produce a competitor-grounded redesign (Google Docs' page-on-gray-canvas metaphor, Notion's grayscale-badge restraint, Linear's single-custom-hue discipline), then confirmed three subjective calls with the user directly rather than assuming: a custom deep-teal token scale (`#216358`/`#1A4F46`, not a stock Tailwind color) over a black-and-white or amber alternative; kept Geist Sans everywhere rather than introducing a serif for document content; confirmed the Clear Formatting addition. Implemented as a pure retoken + layout pass — zero changes to API routes, permissions, or data model; all 15 existing tests still pass unmodified, which was itself a useful check that nothing beyond styling had been touched.
